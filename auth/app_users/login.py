...
from users.subscription_plan import check_if_subscription_plan_expired
...
def log_in_user(username, password):
    ...
    if bcrypt.hashpw(password.encode('utf-8'), user["password"]) == user["password"]:
        if check_if_subscription_plan_expired(username):
        ...
        return server_response(msg=server_status_messages.LOGIN_SUCCESS, code=200, access_token=access_token, refresh_token=refresh_token, expires_in=token_expires_in_hours * 3600)
    else:
        return server_response(msg=server_status_messages.LOGIN_FAILED, code=200)
