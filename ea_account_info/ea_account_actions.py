...
def update_ea_account_platform_db(email, element_actions):
...
def update_ea_account_username_db(email, element_actions):
    element_actions.execute_element_action(elements.SETTINGS_ICON, ElementCallback.CLICK)
...
def get_ea_account_from_db_if_exists(email, password):
    ea_account_from_db = db.ea_accounts_collection.find_one({"email": email})
...
def update_ea_account_coins_earned_db(fab):
...
def update_ea_account_total_runtime_db(fab):
    today = str(datetime.datetime.today().strftime('%d-%m-%Y'))
