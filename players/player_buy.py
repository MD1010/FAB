...
from consts.app import FUTBIN_PLAYER_PRICE_URL,MAX_PRICE,SANEPRICERATIO,FUTHEAD_PLAYER
import requests
import json
...
def min_price_after_prices_sanity_check(player_prices):
    ...
    if player_prices[0] == 0 :
        return MAX_PRICE
    elif player_prices[1] == 0:
        return player_prices[0]
    elif player_prices[2] == 0:
        return player_prices[0] if player_prices[0]/player_prices[1] > SANEPRICERATIO else player_prices[1]
    else:
        if player_prices[0]/player_prices[1] > SANEPRICERATIO:
            return player_prices[0]
        elif player_prices[1]/player_prices[2] > SANEPRICERATIO:
            return player_prices[1]
        else:
            return player_prices[2]
