...
from items.item_search import get_player_to_search
from players.player_cards import get_cards_from_the_same_player
...
def update_search_player_if_coin_balance_changed(fab, player_to_search, requested_players, real_prices):
    ...
    if new_coin_balance != fab.user.coin_balance:
        ...
        player_to_search = get_player_to_search(fab.user.coin_balance, requested_players, real_prices)
        if player_to_search is not None:
            init_new_search(fab.element_actions, player_to_search)
    return player_to_search


def init_new_search(element_actions, player_to_search, price_to_search=None):
